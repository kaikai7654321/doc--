...
window.onload = function(){
	...
	m('name').onblur = function(){
		...
		if(cname1 == 'yes'){
            // xmlhttp = new XMLHttpRequest();
			xmlhttp.open('get','Register?option=username&name='+name,true);
    ...
    m('register').onclick = function(){
            ...
            url = 'Register?option=register&name=' + m('name').value + '&password=' + m('password1').value;
            ...
            xmlhttp.onreadystatechange = function () {
                ...
                if (xmlhttp.readyState == 4) {
                    if (xmlhttp.status == 200) {
                        ...
                        if (msg == '1') {
                            alert('注册成功');
                            window.open('index.jsp?name=' + m('name').value + '&password=' + m('password1').value, '_self', '', false);
