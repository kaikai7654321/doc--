...
<html>
  <head>
	...
	<script type="text/javascript" src="jsp/admin/ShowUser.js"></script>
	<!--
  ...
  <body>
	...
	<%
	ArrayList all = (ArrayList)request.getAttribute("userList");
	...
	<td>${user.password }</td>
	<td><a href="DeleteControl?option=deleteUser&id=${user.userAccount }">删除</a></td>
	...
	<div>
	<a href='DeleteControl?option=showUser&pageNow=<%=(pageNow-1) %>' >上一页</a>
		<%
		}
		for(int i = 0; i < pageAmount; i++)
		{
		 %>	 
				 ...
				 <a href='DeleteControl?option=showUser&pageNow=<%=(i+1) %>' ><<%=(i+1) %>></a>
		<%
		}
		...
		<a href='DeleteControl?option=showUser&pageNow=<%=(pageNow+1) %>' >下一页</a>;
		<%
		}
		 %><br>
