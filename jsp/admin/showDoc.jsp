...
<tr>
	...
	<td><a href="downloadFile?id=${doc.id }" ><img alt="下载啦" src="#"></a></td>
	<td><a href="docCRUD?admin=admin&optionName=delete&id=${doc.id }&pageNow=<%=pageNowTemp %>" ><img alt="删除" src="#"></a></td>
	<!-- 
	...
	if(pageNow!=1)
	{
	 %>
		...
		<a href='user_index?name=showDoc&admin=admin&pageNow=<%=(pageNow-1) %>' >上一页</a>
		<%
		}
		for(int i = 0; i < pageAmount; i++)
		{
		 %>	 
				 ...
				 <a href='user_index?name=showDoc&admin=admin&pageNow=<%=(i+1) %>' ><<%=(i+1) %>></a>
		<%
		}
		...
		<a href='user_index?name=showDoc&admin=admin&pageNow=<%=(pageNow+1) %>' >下一页</a>;
		<%
		}
		 %><br>
